...
void Preprocess::v_m1600_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    pl_surf.clear();
    // pl_corn.clear();
    // pl_full.clear();

    pcl::PointCloud<v_m1600::Point> pl_orig;
    pcl::fromROSMsg(*msg, pl_orig);
    int plsize = pl_orig.points.size();
    if (plsize == 0) return;
    pl_surf.reserve(plsize);
    
    double time_head_sec = pl_orig.points[0].timestampSec;
    double time_head_nsec = pl_orig.points[0].timestampNsec;
